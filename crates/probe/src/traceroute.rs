...
/// `loss_pct` is the share of the hop's probes that didn't reply.
...
pub async fn traceroute(target: &str, max_hops: u8, to: Duration) -> Option<TraceResult> {
    ...
    let mut cmd = {
        ...
        // -n numeric; -m max hops; -q 5 probes/hop so we can compute per-hop loss.
        c.args(["-n", "-q", "5", "-m", &max_hops.to_string(), target]);
        c
    };
...
pub fn parse_traceroute(text: &str) -> Vec<Hop> {
    ...
    for line in text.lines() {
        ...
        let rest = &toks[1..];
        let (rtt_ms, loss_pct) = hop_stats(rest);
        hops.push(Hop { hop_no, ip: first_ip(rest), rtt_ms, loss_pct });
    }
    hops
}
...
/// Per-hop stats over the multiple probes on a hop line: average RTT of the
/// replies, and loss % = share of probes that didn't reply (`*`). Handles the
/// `<1 ms` form (Windows) by stripping a leading `<`.
fn hop_stats(toks: &[&str]) -> (Option<f64>, Option<f64>) {
    let mut rtts = Vec::new();
    for w in toks.windows(2) {
        if w[1] == "ms" {
            if let Ok(v) = w[0].trim_start_matches('<').parse::<f64>() {
                rtts.push(v);
            }
        }
    }
    let stars = toks.iter().filter(|&&t| t == "*").count();
    let probes = rtts.len() + stars;
    let rtt = (!rtts.is_empty()).then(|| rtts.iter().sum::<f64>() / rtts.len() as f64);
    let loss = (probes > 0).then(|| stars as f64 / probes as f64 * 100.0);
    (rtt, loss)
}
...
mod tests {
    ...
    fn parses_gnu_traceroute() {
        // -q 5 style: hop 3 lost 2 of 5 probes (40%).
        let sample = "\
traceroute to 1.1.1.1 (1.1.1.1), 30 hops max, 60 byte packets
 1  192.168.1.1  1.0 ms  2.0 ms  3.0 ms  1.0 ms  1.0 ms
 2  * * * * *
 3  10.0.0.1  5.0 ms  * 5.0 ms  * 5.0 ms
 4  1.1.1.1  9.80 ms";
        let hops = parse_traceroute(sample);
        assert_eq!(hops.len(), 4);
        assert_eq!(hops[0], Hop { hop_no: 1, ip: Some("192.168.1.1".into()), rtt_ms: Some(1.6), loss_pct: Some(0.0) });
        assert_eq!(hops[1], Hop { hop_no: 2, ip: None, rtt_ms: None, loss_pct: Some(100.0) });
        assert_eq!(hops[2].ip.as_deref(), Some("10.0.0.1"));
        assert_eq!(hops[2].rtt_ms, Some(5.0));
        assert_eq!(hops[2].loss_pct, Some(40.0), "2 of 5 probes lost");
...
Trace complete.";
        ...
        assert_eq!(hops[0].loss_pct, Some(0.0));
        assert_eq!(hops[2], Hop { hop_no: 3, ip: None, rtt_ms: None, loss_pct: Some(100.0) });
        assert_eq!(hops[3].ip.as_deref(), Some("1.1.1.1"));
    }

    #[test]
    fn route_hash_changes_with_path() {
        let a = vec![Hop { hop_no: 1, ip: Some("10.0.0.1".into()), rtt_ms: Some(1.0), loss_pct: Some(0.0) }];
        let b = vec![Hop { hop_no: 1, ip: Some("10.0.0.2".into()), rtt_ms: Some(1.0), loss_pct: Some(0.0) }];
